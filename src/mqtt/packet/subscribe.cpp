...
mqtt::subscribe_packet::subscribe_packet(uint16_t pkt_id, std::u8string topic_filter, options_t options, mqtt::properties properties)
    : m_properties(std::move(properties))
{
    m_owned = true;
    m_packet = new packet(mqtt::packet_type::SUBSCRIBE);

    packet& subscribe = *m_packet;

    // Variable Header
    subscribe += pkt_id;
    subscribe += m_properties;
    ...
    : m_properties(std::move(properties))
{
    if(topic_filters.size() == 0 || options.size() == 0) {
        error1("mqtt::subscribe_packet constructor: Topic filters or options cannot be size 0!\n");
        panic("invalid subscribe packet");
    }

    if(topic_filters.size() != options.size()) {
        error1("mqtt::subscribe_packet constructor: Topic filters and options must have same size!\n");
        panic("invalid subscribe packet");
    }
    ...
    for(uint i = 0; i < topic_filters.size(); i++) {
        subscribe += topic_filters[i];
        subscribe += options[i];
    }
}

mqtt::subscribe_packet::subscribe_packet(mqtt::packet* packet) 
    : m_owned(false)
    , m_packet(packet)
    , m_properties(packet->contents().subspan(props_offset()))
{}

mqtt::subscribe_packet::~subscribe_packet() {
    if(m_owned && m_packet) {
        delete m_packet;
    }
}

size_t mqtt::subscribe_packet::id_offset() const {
    return 0;
}

size_t mqtt::subscribe_packet::props_offset() const {
    return 2;
}

size_t mqtt::subscribe_packet::payload_offset() const {
    const mqtt::properties& props = properties();
    return props_offset() + props.m_length + props.m_length.length;
}

mqtt::packet_type mqtt::subscribe_packet::type() const {
    if(!m_packet) {
...
mqtt::varint_t mqtt::subscribe_packet::length() const {
    if(!m_packet) {
        return {0};
    }
...
std::u8string_view mqtt::subscribe_packet::topic_filter(uint index) const {
...
std::optional<mqtt::subscribe_packet::options_t> mqtt::subscribe_packet::options(uint index) const {
    size_t offset = payload_offset();
    std::span<uint8_t> data = m_packet->contents();
    uint16_t length = (data[offset] << 8) | data[offset + 1];
    while(index > 0 && offset < data.size()) {
...
std::span<uint8_t> mqtt::subscribe_packet::serialize() const {
    if(!m_packet) {
        return {};
    }
...
mqtt::packet* mqtt::subscribe_packet::release() {
